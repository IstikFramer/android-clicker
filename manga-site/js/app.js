...
function initReader() {
  ...
  if (endText) {
    if (next) {
      endText.innerHTML = `Глава ${next.num} «${next.title}» уже доступна!`;
    } else if (t.status === "Завершена") {
      endText.innerHTML = `Это была финальная глава «${t.title}».<br>Спасибо, что дочитали до конца! 🌸`;
    } else {
      endText.innerHTML = `Глава ${ch.num + 1} появится по твоему следующему запросу.<br>Просто напиши «дальше»!`;
    }
  }
...
function initComments(t, chNum) {
  ...
  const presetsByTitle = {
    ...
    vd: {
      ...
      4: [
        { name: "яблоко_в_карамели", text: "ЯБЛОКО НА ДВОИХ ПОСЛЕ СОКА НА ДВОИХ — эти двое неслучайно встретились, я плачу 🍎💔→❤️", time: "20 мин назад" },
        { name: "жюри_неправо", text: "Гран-при Вике — ладно, техника. Но приз зрительских симпатий с лепестками на картине Ани — ВОТ ЭТО финал 😭🌸", time: "1 ч назад" },
        { name: "скетчбук_мальчика", text: "Стоп… маленький Кирилл со скетчбуком в флэшбеке?! И Вика со старым фото?! Глава 5 объяснит, я на ушах 🕵️", time: "3 ч назад" },
      ],
      5: [
        { name: "девять_лет", text: "«Девять лет ты возвращала мне обещание, которого не помнила» — Я РЫДАЮ, не подходите ко мне 😭💔", time: "15 мин назад" },
        { name: "тимур_топ", text: "Тимур с его «как гроза в ночи» появился и УНЕС Вику за два панеля. Новая пара, я болею ⚡💙", time: "1 ч назад" },
        { name: "шур_шур", text: "Два карандаша «шур-шур» под вишней как в детстве — лучший финал главы за всю мангу. Кошачьи уши висят рядом 🐱", time: "2 ч назад" },
      ],
      6: [
        { name: "сто_вишен", text: "«Каждая — чьё-то обещание» — Кирилл посадил СТО ВИШЕН и я официально не способна нормально дышать 🌳😭", time: "10 мин назад" },
        { name: "гроза_в_ночи", text: "Тимур повторил «как гроза в ночи» через ДЕСЯТЬ ЛЕТ перед её картиной в Париже. Их свадьба была где-то между, и я хочу спин-офф ⚡💍", time: "1 ч назад" },
        { name: "полный_круг", text: "ЕВА. ПРОТЯНУЛА. ЛИСТОК. «Теперь под ним двое» — ЭТО ЖЕ СТРАНИЦА ИЗ ГЛАВЫ 5, АВТОР, КАК ТАК МОЖНО 😭🌸 КОНЕЦ ИДЕАЛЕН", time: "2 ч назад" },
      ],
    },
  };
