...
**Добавлено**
- Центр безопасности: Защитник Windows, брандмауэр, UAC, Secure Boot и обновления
- Безопасный анализ необычных команд в автозагрузке
- Умные уведомления только о важных проблемах, без повторного спама
- Десять новых круглых 3D-иконок безопасности и состояния

**Улучшено**
- Каждый пункт показывает простой статус, технические подробности и рекомендацию
- Проверка выполняется в фоне не чаще раза в сутки и ничего не меняет в Windows
- Нажатие на уведомление открывает нужный раздел программы
...
Готовая сборка не требует Python. Скачайте `LIFE-OS-0.4.5-setup.exe` со страницы релизов и запустите его.
