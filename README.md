...
**Добавлено**
- «Центр состояния»: быстрая проверка дисков, сети, автозагрузки и ресурсов
- Понятные рекомендации для пунктов, которым требуется внимание

**Исправлено**
- Кнопка «Готово» больше не накладывается на действия в окне очистки
- Подсказки «?» показывают длинный текст полностью
- Карточка «Сеть» сохраняет ту же ширину, что остальные разделы
- Скруглённые углы больше не оставляют острых тёмных участков

**Улучшено**
- Ползунок прозрачности не блокирует интерфейс во время перетаскивания
- Нижняя панель очистки корректно сбрасывается между проверками
- Развёрнутое окно прилегает к краям экрана без щелей
...
Готовая сборка не требует Python. Скачайте `LIFE-OS-0.4-setup.exe` со страницы релизов и запустите его.
Установщик предложит папку, создаст ярлык в меню «Пуск» и по желанию —
ярлык на рабочем столе. ZIP в релизе предназначен для встроенного обновления
и переносного запуска.
