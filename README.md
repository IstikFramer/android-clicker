...
- Добавлен Setup.exe с выбором папки, меню «Пуск» и опциональным ярлыком на рабочем столе
- Встроенное обновление устанавливает полный Windows ZIP вместе с `_internal` и `python312.dll`
- Перед заменой программа проверяет комплектность библиотек; защищённая папка запрашивает UAC
...
Готовая сборка не требует Python. Скачайте `LIFE-OS-0.3.6-setup.exe` со страницы релизов и запустите его.
