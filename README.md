...
Готовый `LIFE OS.exe` не требует Python: скачайте его со страницы релизов и
запустите. Собирается автоматически на Windows через GitHub Actions
...
Раздел «Инструменты» наводит порядок на диске. Любой из них работает в две
фазы: сначала **сканирование** (ничего не меняется), затем — по вашей команде —
удаление.
